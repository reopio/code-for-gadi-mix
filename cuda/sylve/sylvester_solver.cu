...
struct SparseMatrix {
    ...
    cusparseConstSpMatDescr_t matDescr_half;
    cusparseConstSpMatDescr_t matDescr_float;
};
...
int cg_half_precision(cusparseHandle_t cusparseHandle, cublasHandle_t cublasHandle, 
    ...
    CHECK_CUSPARSE(cusparseSpMV_bufferSize(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE,
                                           &one_ht, A.matDescr_half, vecP, &zero_ht, vecAp,
    ...
    CHECK_CUSPARSE(cusparseDestroySpMat(A.matDescr_half));
    CHECK_CUSPARSE(cusparseCreateConstCsr(&A.matDescr_half, A.rows, A.cols, A.nnz,
...
int cg_float_precision(cusparseHandle_t cusparseHandle, cublasHandle_t cublasHandle, 
    ...
    // Get buffer size
    CHECK_CUSPARSE(cusparseSpMV_bufferSize(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE,
                                           &one_ht, A.matDescr_float, vecP, &zero_ht, vecAp,
                                           CUDA_R_32F, CUSPARSE_SPMV_ALG_DEFAULT, &bufferSize));
    if (bufferSize > 0) {
        CHECK_CUDA(cudaMalloc(&dBuffer, bufferSize));
    }
    ...
    cudaFree(d_Ap);
    if (dBuffer) cudaFree(dBuffer);
    CHECK_CUSPARSE(cusparseDestroySpMat(A.matDescr_float));
    CHECK_CUSPARSE(cusparseCreateConstCsr(&A.matDescr_float, A.rows, A.cols, A.nnz,
...
int cg_double_precision(cusparseHandle_t cusparseHandle, cublasHandle_t cublasHandle,
    ...
    void *dBuffer = nullptr;
    CHECK_CUSPARSE(cusparseSpMV_bufferSize(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE,
                                            &one, A.matDescr, vecP, &zero, vecAp,
    ...
    cudaFree(d_Ap);
    if (dBuffer) cudaFree(dBuffer);
    CHECK_CUSPARSE(cusparseDestroySpMat(A.matDescr));
    CHECK_CUSPARSE(cusparseCreateConstCsr(&A.matDescr, A.rows, A.cols, A.nnz,
...
int half_precision_solver(cusparseHandle_t cusparseHandle, cublasHandle_t cublasHandle,
    ...
    CHECK_CUSPARSE(cusparseSpMV_bufferSize(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE,
                                            &neg_one, A.matDescr, vecX0, &one, vecR,
                                            CUDA_R_64F, CUSPARSE_SPMV_ALG_DEFAULT, &bufferSize));
    if (bufferSize > 0) {
        CHECK_CUDA(cudaMalloc(&dBuffer, bufferSize));
    }
    ...
    CHECK_CUSPARSE(cusparseSpMV_bufferSize(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE,
                                            &one_f, S.matDescr_half, vecR1, &zero_f, vecTemp,
...
int float_precision_solver(cusparseHandle_t cusparseHandle, cublasHandle_t cublasHandle,
    ...
    CHECK_CUSPARSE(cusparseSpMV_bufferSize(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE,
                                            &one_f, S.matDescr_float, vecR1, &zero_f, vecTemp,
                                            CUDA_R_32F, CUSPARSE_SPMV_ALG_DEFAULT, &bufferSize1));
    if (bufferSize1 > 0) {
        CHECK_CUDA(cudaMalloc(&dBuffer1, bufferSize1));
    }
...
int double_precision_solver(cusparseHandle_t cusparseHandle, cublasHandle_t cublasHandle,
    ...
    CHECK_CUSPARSE(cusparseSpMV_bufferSize(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE,
                                               &one, S.matDescr, vecR1, &zero, vecTemp,
...
void createIdentityMatrix(SparseMatrix &I, int n) {
    ...
    CHECK_CUSPARSE(cusparseCreateConstCsr(&I.matDescr, I.rows, I.cols, I.nnz,
                                     I.d_csrRowPtr, I.d_csrColInd, I.d_csrVal,
                                     CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I,
                                     CUSPARSE_INDEX_BASE_ZERO, CUDA_R_64F));
    
    CHECK_CUSPARSE(cusparseCreateConstCsr(&I.matDescr_half, I.rows, I.cols, I.nnz,
                                     I.d_csrRowPtr, I.d_csrColInd, I.d_csrVal_half,
                                     CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I,
                                     CUSPARSE_INDEX_BASE_ZERO, CUDA_R_16F));

    CHECK_CUSPARSE(cusparseCreateConstCsr(&I.matDescr_float, I.rows, I.cols, I.nnz,
                                     I.d_csrRowPtr, I.d_csrColInd, I.d_csrVal_float,
                                     CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I,
                                     CUSPARSE_INDEX_BASE_ZERO, CUDA_R_32F));
}
...
void transposeMatrix(cusparseHandle_t cusparseHandle, const SparseMatrix &A, SparseMatrix_double &AT) {
    ...
    // Create descriptor for the transposed matrix
    CHECK_CUSPARSE(cusparseCreateConstCsr(&AT.matDescr, AT.rows, AT.cols, AT.nnz,
                                     AT.d_csrRowPtr, AT.d_csrColInd, AT.d_csrVal,
                                     CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I,
                                     CUSPARSE_INDEX_BASE_ZERO, CUDA_R_64F));
...
void matrixAdd(cusparseHandle_t cusparseHandle, cublasHandle_t cublasHandle,
    ...
    if constexpr (needs_convert_conversion<Type3>::value) {
        ...
        CHECK_CUSPARSE(cusparseCreateConstCsr(&C.matDescr_half, C.rows, C.cols, C.nnz,
                                         C.d_csrRowPtr, C.d_csrColInd, C.d_csrVal_half,
                                         CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I,
                                         CUSPARSE_INDEX_BASE_ZERO, CUDA_R_16F));

        // Convert to float precision
        double_to_float_kernel<<<blocksPerGrid, threadsPerBlock>>>(C.d_csrVal, C.d_csrVal_float, C.nnz);
        CHECK_CUSPARSE(cusparseCreateConstCsr(&C.matDescr_float, C.rows, C.cols, C.nnz,
                                         C.d_csrRowPtr, C.d_csrColInd, C.d_csrVal_float,
                                         CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I,
                                         CUSPARSE_INDEX_BASE_ZERO, CUDA_R_32F));
    }
    
    // Create descriptors for generic API (常规精度)
    CHECK_CUSPARSE(cusparseCreateConstCsr(&C.matDescr, C.rows, C.cols, C.nnz,
                                     C.d_csrRowPtr, C.d_csrColInd, C.d_csrVal,
                                     CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I,
                                     CUSPARSE_INDEX_BASE_ZERO, CUDA_R_64F));
...
void matrixMultiply(cusparseHandle_t cusparseHandle, const SparseMatrix &A, const SparseMatrix_double &B, SparseMatrix &C) {
    ...
    // Create generic descriptors for C (double and half)
    CHECK_CUSPARSE(cusparseCreateConstCsr(&C.matDescr, C.rows, C.cols, C.nnz,
                                     C.d_csrRowPtr, C.d_csrColInd, C.d_csrVal,
                                     CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I,
                                     CUSPARSE_INDEX_BASE_ZERO, CUDA_R_64F));
...
int main(int argc, char **argv) {
    ...
    void *dBuffer = nullptr;
    CHECK_CUSPARSE(cusparseSpMV_bufferSize(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE,
                                           &one, A.matDescr, vecOnes, &zero, vecB,
                                           CUDA_R_64F, CUSPARSE_SPMV_ALG_DEFAULT, &bufferSize));
    if (bufferSize > 0) {
        CHECK_CUDA(cudaMalloc(&dBuffer, bufferSize));
    }
    ...
    // cusparseDestroySpMat(I_double.matDescr);
    cusparseDestroySpMat(A.matDescr);
    cusparseDestroySpMat(A.matDescr_half);
    cusparseDestroySpMat(AH.matDescr);
    cusparseDestroySpMat(AH.matDescr_half);
    cusparseDestroySpMat(ASS.matDescr);
    cusparseDestroySpMat(ASS.matDescr_half);
    cusparseDestroySpMat(S.matDescr);
    cusparseDestroySpMat(I.matDescr);
    cusparseDestroySpMat(I.matDescr_half);
    cusparseDestroySpMat(ALPHA_I_MINUS_S.matDescr);
    cusparseDestroySpMat(ALPHA_I_MINUS_S.matDescr_half);
    cusparseDestroySpMat(AT.matDescr);
    cusparseDestroySpMat(H.matDescr);
    cusparseDestroySpMat(ASX.matDescr);
    
    cusparseDestroyDnVec(vecOnes);
